<?php
/*
Plugin Name: ShoutSuite
Plugin URI: http://sagad.net/
Description: A WordPress Plugin to integrate ngeSHOUT and WordPress
Version: 1.2
...
register_activation_hook(__FILE__,'shoutbacks_install');
...
if( is_single() ){
	...
	foreach ($buff as $line){
		...
		if(get_option('shoutsuite_reshoutthis')){
			...
			$rt_this = WP_CONTENT_URL.'/plugins/'.plugin_basename(dirname(__FILE__)).'/rs_this.gif';		  
	...
	if(get_option('shoutsuite_shoutthis')){
		...
		if (file_exists(WP_CONTENT_URL.'/plugins/'.plugin_basename(dirname(__FILE__)).'/'.$fn)){
			...
			$src = WP_CONTENT_URL.'/plugins/'.plugin_basename(dirname(__FILE__))."/rs-gif.php?count=$sb_count";
		}	
...
?>
<div class="wrap">
<h2>ShoutSuite <?php echo $ts_version;?></h2>
